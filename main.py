...
total_salary_fig.update_layout(
    title='Средняя зарплата без учета инфляции',
    xaxis_title='Год',
...
# Настройка внешнего вида графика
fig.update_layout(
    title='Средняя зарплата с учетом инфляции в рублях 2000 года',
    xaxis_title='Год',
    yaxis_title='Зарплата',
    legend_title='Сектор',
    xaxis=dict(tickangle=45),
...
st.write("""
    **Рост инфляции**:
    ...
    - Подъем в 2021 году вызван ростом спроса после смягчения карантинных мер из-за пандемии COVID-19.
...
Некоторые компании могут реагировать на инфляцию, увеличивая зарплаты своих сотрудников в соответствии с индексом потребительских цен. Однако в экономически нестабильных условиях этого не происходит.
...
По итогу проекта видно, что с 2000 по 2023 годы рубль сильно обесценился, потеряв более 80% своей цены, что снизило рост доходов работников.
""")
...
st.write("""
ВВП России в 2000-е годы демонстрировал стабильный рост вместе с доходами населения. Корреляция положительная. 
...
В 2019 году была пандемия COVID-19 и снижение цен на энергоносители. В официальной статистике данный период не отразился падением доходов. Причина в том, что были увольнения людей и это не влияло на оклады работающего населения.
""")
